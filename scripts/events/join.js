module.exports.config = {
  ...
  version: "1.0.0",
  credits: "Mirai-Team", // FIXED BY JOY
...
module.exports.run = async function({ api, event, Users }) {
  ...
  if (event.logMessageData.addedParticipants && Array.isArray(event.logMessageData.addedParticipants) && event.logMessageData.addedParticipants.some(i => i.userFbId == api.getCurrentUserID())) {
    ...
    let gifUrl = 'https://i.imgur.com/wFah8uN.jpeg';
let gifPath = __dirname + '/Joy/join/join.gif';
...
.catch(error => {
    ...
    try {
      if (!fs.existsSync(__dirname + `/ryuko/font/Semi.ttf`)) {
        let getfont = (await axios.get(fontlink, { responseType: "arraybuffer" })).data;
        fs.writeFileSync(__dirname + `/ryuko/font/Semi.ttf`, Buffer.from(getfont, "utf-8"));
      };
      ...
      for (let o = 0; o < event.logMessageData.addedParticipants.length; o++) {
        let pathImg = __dirname + `/Joy/join/${o}.png`;
        let pathAva = __dirname + `/Joy/join/avt.png`;
        ...
        registerFont(__dirname + `/ryuko/font/Semi.ttf`, {
          family: "Semi"
        });
        ...
        abx.push(fs.createReadStream(__dirname + `/Joy/join/${o}.png`))
      }
      ...
      for (let ii = 0; ii < parseInt(id.length); ii++) {
        fs.unlinkSync(__dirname + `/Joy/join/${ii}.png`)
      }
